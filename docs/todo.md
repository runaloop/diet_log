...
## Инструкция и доки
- [ ] Убрать механику префикса `!` («разовое блюдо»): на практике ей не пользовались, а агентов она сбивала постоянно. Места: `AGENTS.md:54, 61, 71`, `.claude/agents/product-lookup.md:34, 41`, `lstrip('!')` в `scripts/log.py:214`. Отдельного кодового пути у неё нет — признак разового блюда и так `--k/--b/--zh/--u`: есть явные КБЖУ → строка пишется как названа и в каталог не идёт. Это и оставить единственным правилом.
